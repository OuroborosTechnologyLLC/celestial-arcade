    ...
    event.waitUntil(
        caches.keys().then((cacheNames) => {
            return Promise.all(cacheNames.map((cacheName) => {
                if (cacheName !== STATIC_CACHE && !cacheName.startsWith(GAME_CACHE_PREFIX)) return caches.delete(cacheName);
            }));
        })
    );
    self.clients.claim();
...
async function cacheGame(gameSlug, manifest) {
    ...
    if (failures.length > 0) {
        self.clients.matchAll().then(clients => { clients.forEach(client => client.postMessage({ type: 'CACHE_ERROR', gameSlug, failures: failures.length })); });
    } else {
        self.clients.matchAll().then(clients => { clients.forEach(client => client.postMessage({ type: 'CACHE_COMPLETE', gameSlug })); });
    }
}
...
async function deleteGameCache(gameSlug) {
    ...
    self.clients.matchAll().then(clients => { clients.forEach(client => client.postMessage({ type: 'CACHE_DELETED', gameSlug })); });
}
